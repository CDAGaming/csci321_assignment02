...
namespace MarbleGame
{
    public class Board
    {
        ...
        //initializes N M W
        public void InitializeBasicParams(string strData = "", string accessMode = "console")
        {
            string[] values = ReadValues(strData, 0, accessMode);
            if (values.Length < 3)
        ...
        public void InitializeBoardObjects(BoardObjectTypes objectType, string strData = "", string accessMode = "console")
        {
            for (int i = 0; i < MarblesCount; i++)
            {
                Console.Write(objectType.ToString() + " {0}. ", i + 1);
                string[] values = ReadValues(strData, (objectType == BoardObjectTypes.Hole ? MarblesCount : 0) + i + 2, accessMode);
                if (values.Length < 2)
                ...
                if (objectType == BoardObjectTypes.Marble)
        ...
        //initializes walls
        public void InitializeBoardObjects(string strData = "", string accessMode = "console")
        {
            for (int i = 0; i < WallsCount; i++)
            {
                Console.Write("Wall {0}. ", i + 1);
                string[] values = ReadValues(strData, (MarblesCount * 2) + i + 2, accessMode);
                if (values.Length < 4)
        ...
        private string[] ReadValues(string strData = "", int fileLine = 0, string accessMode = "console")
        {
            string positionString;
            switch (accessMode)
            {
                case "file":
                    string line = File.ReadLines(strData).Skip(fileLine - 1).Take(1).First();
                    positionString = line;
                    Console.WriteLine("Using Value from File: " + line);
                    break;
                case "raw":
                    positionString = strData;
                    Console.WriteLine("Using Value: " + strData);
                    break;
                default:
        ...
        public Solution LiftBoard(Sides liftedSide, string moves, bool allowFailues = false, bool doContinue = false)
        {
            ...
            if (response.Success && !newBoardState.BoardStateDidNotChange(response.BoardState))
            {
                ...
                if (response.Finished)
                    return new Solution { Success = true, Route = moves, BoardState = newBoardState };
                else
                {
                    ...
                    {
                        return new Solution { Success = false, Route = moves, BoardState = newBoardState };
                    }
                }
            }
